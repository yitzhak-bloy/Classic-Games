...
const Statistics = () => {
  ...
  useEffect(() => {
    const req = async () => {
      try {
        allUsers = await sendRequest(
          "http://localhost:5000/api/userStatistics/"
        );
  ...
  const rowsPublic = [
    createData(
      "Heard",
      `${userHard1.name}: [${userHard1.ticTacToeStatistic.hard.averageRating}]`,
      `${userHard2.name}: [${userHard2.ticTacToeStatistic.hard.averageRating}]`,
      `${userHard3.name}: [${userHard3.ticTacToeStatistic.hard.averageRating}]`
    ),
    createData(
      "easy",
      `${userEasy1.name}: [${userEasy1.ticTacToeStatistic.easy.averageRating}]`,
      `${userEasy2.name}: [${userEasy2.ticTacToeStatistic.easy.averageRating}]`,
      `${userEasy3.name}: [${userEasy3.ticTacToeStatistic.easy.averageRating}]`
    ),
  ];
  ...
  if (emailOfUser) {
    ...
    rowsPersonal = [
      createData(
        "Heard",
        userSta.ticTacToeStatistic.hard.averageRating,
        userSta.ticTacToeStatistic.hard.victory,
        userSta.ticTacToeStatistic.hard.loss,
        userSta.ticTacToeStatistic.hard.draw
      ),
      createData(
        "easy",
        userSta.ticTacToeStatistic.easy.averageRating,
        userSta.ticTacToeStatistic.easy.victory,
        userSta.ticTacToeStatistic.easy.loss,
        userSta.ticTacToeStatistic.easy.draw
      ),
    ];
  }
  ...
  return (
    <div>
      ...
      {!emailOfUser ? (
        <h1 align='center'>
          To see the personal score, you need to{" "}
